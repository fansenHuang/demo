...
function formSubmit(url,sTarget){
    document.forms[0].action = url;  //设置跳转路径，相对路径
    document.forms[0].target = sTarget;//页面打开方式
...
//根据验证结果来确实是否提交表单
function formSubmitWithValidate (url,sTarget,validateResult){
	if(validateResult){
...
//计算指定name值的一批复选框的选中个数,"deptId"
function getCheckBoxCheckedCount(name){
...
//验证指定name值的复选框的选中个数是否为1(用于查看，修改操作)
function validateOne(name){
	var index = getCheckBoxCheckedCount(name);
	if(index===0){
		layer.msg('没有选中任何行！');
		return false;
	}else if(index>1){
...
//验证指定name值的复选框的选中个数是否>=1(用于删除，启用，停用操作)
function validateOneOrMore(name){
	var index = getCheckBoxCheckedCount(name);
	if(index===0){
		layer.msg('没有选中任何行！');
		return false;
